    ...
    <main class="flex-1 flex flex-col overflow-hidden">
        ...
        <header
            ...
            <div class="flex items-center gap-4">
                ...
                <a href="{{ url('/sdm/pengajuan-masuk') }}"
                    class="w-10 h-10 rounded-xl border border-slate-200 bg-slate-50 flex items-center justify-center text-slate-500 hover:kai-text-orange hover:border-orange-200 transition-all shadow-sm"
                    title="Kembali ke Daftar">
                    <i class="fa-solid fa-arrow-left"></i>
                </a>
                <div>
                    <h2 class="text-2xl font-black text-[#0b1739] tracking-tight">Detail Pengajuan Masuk</h2>
                    <p class="text-xs font-semibold text-slate-400 mt-0.5">Informasi lengkap biodata pendaftar beserta
                        berkas dokumen persyaratan administrasi</p>
                </div>
            </div>
        ...
        <div
            ...
            <div class="relative z-10 space-y-6">
                ...
                @if(session('success'))
                    <div
                        class="bg-emerald-50 border border-emerald-200 text-emerald-700 text-sm font-semibold px-5 py-3 rounded-xl">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
                    ...
                    <div
                        class="bg-white rounded-2xl border border-slate-200/50 p-6 shadow-xl shadow-blue-950/10 lg:col-span-2 relative overflow-hidden transition-shadow hover:shadow-2xl">
                        <div class="flex justify-between items-center border-b border-slate-100 pb-4 mb-4">
                            <h3
                                class="text-sm font-extrabold text-[#0b1739] uppercase tracking-wider flex items-center gap-2">
                                <i class="fa-solid fa-id-card kai-text-orange"></i> Biodata Mahasiswa
                            </h3>
                            <span
                                class="inline-flex text-[11px] font-extrabold px-3 py-1 bg-amber-50 text-amber-600 border border-amber-200 rounded-lg uppercase tracking-wider">
                        ...
                        <div class="divide-y divide-slate-100 text-sm">
                            <div class="py-3 grid grid-cols-3"><span class="text-slate-400 font-bold">Nama</span><span
                                    class="col-span-2 font-extrabold text-slate-800 text-[15px]">{{ $pengajuan->nama ?? '-' }}</span>
                            </div>
                            <div class="py-3 grid grid-cols-3"><span class="text-slate-400 font-bold">NIM</span><span
                                    class="col-span-2 font-bold text-slate-700 tracking-wide">{{ $pengajuan->nim ?? '-' }}</span>
                            </div>
                            <div class="py-3 grid grid-cols-3"><span class="text-slate-400 font-bold">Email</span><span
                                    class="col-span-2 font-medium text-slate-600">{{ $pengajuan->email ?? '-' }}</span>
                            </div>
                            <div class="py-3 grid grid-cols-3"><span class="text-slate-400 font-bold">No. HP</span><span
                                    class="col-span-2 font-medium text-slate-600">{{ $pengajuan->no_hp ?? '-' }}</span>
                            </div>
                            <div class="py-3 grid grid-cols-3"><span
                                    class="text-slate-400 font-bold">Universitas</span><span
                                    class="col-span-2 font-bold text-slate-700">{{ $pengajuan->universitas ?? '-' }}</span>
                            </div>
                            <div class="py-3 grid grid-cols-3"><span class="text-slate-400 font-bold">Jurusan /
                                    Prodi</span><span
                                    class="col-span-2 font-medium text-slate-600">{{ $pengajuan->jurusan ?? '-' }}</span>
                            </div>
                            <div class="py-3 grid grid-cols-3"><span class="text-slate-400 font-bold">Unit
                                    Tujuan</span><span
                                    class="col-span-2 font-extrabold text-blue-700 bg-blue-50 px-3 py-1 rounded-xl w-fit text-xs border border-blue-100">{{ $pengajuan->unit_tujuan ?? 'Belum Ditentukan' }}</span>
                            </div>
                            <div class="py-3 grid grid-cols-3"><span class="text-slate-400 font-bold">Tanggal
                                    Pengajuan</span><span class="col-span-2 font-medium text-slate-500"><i
                                        class="fa-solid fa-calendar text-slate-300 mr-1"></i>
                                    {{ $pengajuan->tanggal_pengajuan ?? '-' }}</span></div>
                            <div class="py-4 grid grid-cols-3"><span
                                    class="text-slate-400 font-bold">Motivasi</span><span
                                    class="col-span-2 text-xs font-semibold text-slate-500 bg-slate-50 p-3 rounded-xl border border-slate-200/60 leading-relaxed shadow-inner">{{ $pengajuan->motivasi ?? '-' }}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div
                        class="bg-white rounded-2xl border border-slate-200/50 p-6 shadow-xl shadow-blue-950/10 flex flex-col gap-4 transition-shadow hover:shadow-2xl">
                        <h3
                            class="text-sm font-extrabold text-[#0b1739] uppercase tracking-wider border-b border-slate-100 pb-3 flex items-center gap-2">
                            <i class="fa-solid fa-gavel kai-text-orange"></i> Aksi Keputusan SDM
                        </h3>

                        <form id="statusForm"
                            action="{{ isset($pengajuan) ? url('/sdm/pengajuan/' . $pengajuan->id . '/update-status') : '#' }}"
                            ...
                            <!-- Teruskan ke Unit -->
                            <button type="button"
                                class="action-btn w-full bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold py-3.5 px-4 rounded-xl shadow-md shadow-emerald-900/20 transition-all flex items-center justify-center gap-2 text-xs uppercase tracking-wider"
                            ...
                            <!-- Minta Revisi -->
                            <button type="button"
                                class="action-btn w-full kai-bg-orange hover:bg-orange-600 text-white font-extrabold py-3.5 px-4 rounded-xl shadow-md shadow-orange-900/20 transition-all flex items-center justify-center gap-2 text-xs uppercase tracking-wider"
                            ...
                            <!-- Tolak -->
                            <button type="button"
                                class="action-btn w-full bg-rose-600 hover:bg-rose-700 text-white font-extrabold py-3.5 px-4 rounded-xl shadow-md shadow-rose-900/20 transition-all flex items-center justify-center gap-2 text-xs uppercase tracking-wider"
                                data-status="ditolak">
                ...
                <div
                    class="bg-white rounded-2xl border border-slate-200/50 p-6 shadow-xl shadow-blue-950/10 transition-shadow hover:shadow-2xl">
                    <div class="flex justify-between items-center border-b border-slate-100 pb-4 mb-4">
                        <h3
                            class="text-sm font-extrabold text-[#0b1739] uppercase tracking-wider flex items-center gap-2">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        @forelse(($berkas ?? []) as $b)
                            <div
                                class="p-4 bg-slate-50/60 rounded-xl border border-slate-200/70 flex justify-between items-center hover:bg-slate-100 hover:border-slate-300 transition-all group">
                                <div class="flex flex-col gap-1 pr-2">
                                    <span
                                        class="text-xs font-extrabold text-slate-800 tracking-tight leading-tight">{{ $b->nama_berkas }}</span>
                                ...
                                @if($b->download_url)
                                    <a href="{{ $b->download_url }}" target="_blank" rel="noopener"
                                        class="w-9 h-9 rounded-lg bg-blue-50 text-blue-600 border border-blue-100 flex items-center justify-center hover:kai-bg-navy hover:text-white hover:border-transparent transition-all shadow-sm shrink-0"
                                        title="Download File">
                                        <i class="fa-solid fa-cloud-arrow-down text-sm"></i>
                                    </a>
                                @else
                                    <span
                                        class="w-9 h-9 rounded-lg bg-slate-100 text-slate-300 border border-slate-200 flex items-center justify-center shrink-0 cursor-not-allowed"
                        ...
                        @empty
                            <p class="text-xs font-semibold text-slate-400 italic col-span-full">Belum ada data jenis berkas
                                dari backend.</p>
                        @endforelse
                    </div>
                </div>
    ...
    <script>
        ...
        document.querySelectorAll('.action-btn').forEach(btn => {
            ...
            btn.addEventListener('click', async () => {
                ...
                const label = {
                    disposisi: "teruskan ke unit",
                    perlu_perbaikan: "meminta revisi",
                    ditolak: "menolak"
                };
                ...
                if (status === "perlu_perbaikan") {
                    ...
                    const hasil = await Swal.fire({
                        ...
                        input: 'textarea',
                        heightAuto: false,
                        inputLabel: 'Tuliskan catatan revisi',
                        inputPlaceholder: 'Misalnya: Surat pengantar belum ditandatangani...',
                        inputAttributes: {
                            rows: 6
                        },
                ...
                const konfirmasi = await Swal.fire({
                    title: "Konfirmasi",
                    text: `Apakah Anda yakin ingin ${label[status]} pengajuan ini?`,
                    icon: "question",
                    heightAuto: false,
                    showCancelButton: true,
