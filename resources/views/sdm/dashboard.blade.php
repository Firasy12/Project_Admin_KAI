        ...
        <header class="h-24 bg-white border-b border-slate-200/80 flex items-center justify-between px-8 shrink-0 shadow-sm relative z-10">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xs font-semibold text-slate-400 mt-0.5">Pantau, verifikasi, dan kelola alur administrasi penempatan magang mahasiswa</p>
                </div>
            </div>
            ...
            <div class="flex items-center space-x-4">
                <div class="flex items-center bg-slate-50 border border-slate-200/80 rounded-2xl p-2 pr-5 shadow-inner">
                    <div class="w-9 h-9 rounded-xl kai-bg-navy text-white flex items-center justify-center font-black text-sm tracking-wide mr-3 shadow-md shadow-blue-950/30">
                        AS
                    </div>
    ...
    <div class="bg-white rounded-2xl p-5 flex items-center justify-between shadow-xl shadow-blue-950/5 border border-blue-100/30 relative overflow-hidden transition-all duration-300 ease-out hover:-translate-y-1.5 hover:shadow-2xl hover:shadow-blue-950/10 group">
        ...
        <span class="text-[10px] font-black px-2.5 py-1 bg-rose-100 text-rose-700 border border-rose-200 rounded-lg shadow-sm uppercase tracking-wide relative z-10 transition-colors group-hover:bg-rose-600 group-hover:text-white group-hover:border-transparent">Gagal</span>
                ...
                <div class="bg-white rounded-2xl border border-slate-200 shadow-md overflow-hidden transition-shadow hover:shadow-lg">
                    ...
                    <div class="p-6 flex justify-between items-center border-b border-slate-100 bg-gradient-to-r from-slate-50 to-white relative">
                        ...
                        <div>
                            ...
                            <p class="text-xs font-semibold text-slate-400 mt-0.5">Log data berkas registrasi mahasiswa yang baru saja masuk ke sistem antrean</p>
                        </div>
                        <a href="{{ url('/sdm/pengajuan-masuk') }}" class="kai-bg-navy hover:kai-bg-orange text-white text-xs font-bold px-4 py-2.5 rounded-xl transition-all shadow-md shadow-blue-950/20 flex items-center gap-2 group">
                        ...
                        <table class="w-full text-left border-collapse">
                            <thead>
                                <tr class="kai-bg-navy text-slate-300 text-[11px] font-bold uppercase tracking-wider">
                                    <th class="px-6 py-4 rounded-tl-sm border-r border-slate-800/30">Mahasiswa / Institusi</th>
                                ...
                                @if(isset($pengajuan_terbaru) && count($pengajuan_terbaru) > 0)
                                    ...
                                    <tr class="hover:bg-orange-50/20 even:bg-slate-50/40 transition-all duration-150 border-l-4 border-transparent hover:border-l-[#f47920]">
                                        ...
                                        <td class="px-6 py-4 text-xs font-bold text-slate-400 whitespace-nowrap">
                                            <span class="flex items-center gap-1.5 text-slate-500"><i class="fa-solid fa-clock text-slate-300 text-sm"></i>{{ $item->created_at ? $item->created_at->format('d M Y, H:i') : '-' }}</span>
                                        </td>
                                    </tr>
                                    @endforeach
                                @else
                                    {{-- LAYOUT BARIS DETIL (MOCKUP) --}}
                                    <tr class="hover:bg-orange-50/20 transition-all duration-150 border-l-4 border-transparent hover:border-l-[#f47920]">
                                        <td class="px-6 py-4">
                                            <div class="font-extrabold text-slate-800 text-[14px]">Nizam Kori</div>
                                            <div class="text-xs text-slate-400 font-bold mt-1 flex items-center gap-1.5">
                                        ...
                                        <td class="px-6 py-4">
                                            <span class="inline-flex items-center gap-1.5 text-xs font-bold px-3 py-1 rounded-xl bg-blue-50 text-blue-700 border border-blue-100 shadow-sm">
                                                <span class="w-1.5 h-1.5 rounded-full bg-blue-500 animate-pulse"></span>Unit Sistem Informasi
                                            </span>
                                        </td>
                                        <td class="px-6 py-4 text-xs font-bold text-slate-500 whitespace-nowrap">
                                            <span class="flex items-center gap-1.5"><i class="fa-solid fa-calendar text-slate-300 text-sm"></i>03 Jul 2026, 15:15</span>
                                        </td>
                                    </tr>
                                @endif
                            </tbody>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div class="bg-white rounded-2xl border border-slate-200 p-6 shadow-md flex flex-col justify-between min-h-[290px] relative">
                        ...
                        <div class="flex items-center justify-start gap-8 py-4 mt-2">
                            ...
                            <div class="relative flex items-center justify-center shrink-0">
                                <div class="w-32 h-32 rounded-full border-[12px] border-slate-100 border-t-[#f47920] border-r-[#0b1739] border-b-emerald-500 flex flex-col items-center justify-center shadow-lg bg-slate-50/50">
                                    <span class="text-2xl font-black text-[#0b1739]">85%</span>
                                    <span class="text-[8px] font-black text-slate-400 uppercase tracking-widest">Terisi</span>
                                </div>
                            </div>
                            
                            {{-- Keterangan Warna List Samping --}}
                            <div class="grid grid-cols-1 gap-2 text-xs font-bold text-slate-600">
                                <div class="flex items-center"><span class="w-3 h-3 rounded-md bg-blue-600 mr-2 shadow-sm"></span> Masuk</div>
                                <div class="flex items-center"><span class="w-3 h-3 rounded-md bg-purple-600 mr-2 shadow-sm"></span> Review</div>
                                <div class="flex items-center"><span class="w-3 h-3 rounded-md bg-emerald-500 mr-2 shadow-sm"></span> Diterima</div>
                                <div class="flex items-center"><span class="w-3 h-3 rounded-md bg-rose-500 mr-2 shadow-sm"></span> Ditolak</div>
                            </div>
                        </div>
                    </div>

                    <!-- Sistem Log Aktivitas Sistem (2 Kolom) -->
                    <div class="bg-white rounded-2xl border border-slate-200 p-6 lg:col-span-2 shadow-md flex flex-col justify-between min-h-[290px]">
                        <div>
                            <div class="border-l-4 border-l-[#0b1739] pl-3 mb-4">
                                <h5 class="text-base font-extrabold text-[#0b1739] tracking-tight">Log Histori Aktivitas</h5>
                            ...
                            <div class="space-y-3 max-h-44 overflow-y-auto pr-1 custom-scrollbar">
                                <div class="flex items-start gap-3.5 p-3.5 bg-gradient-to-r from-slate-50 to-white rounded-xl border border-slate-200/70 shadow-sm hover:border-blue-200 transition-colors">
                                    <div class="w-9 h-9 rounded-xl bg-blue-50 border border-blue-100 text-blue-600 flex items-center justify-center text-sm shrink-0 shadow-sm"><i class="fa-solid fa-user-plus"></i></div>
                                    <div>
                                        <p class="text-xs font-black text-slate-800">Sinkronisasi Berkas Masuk</p>
                                        <p class="text-[11px] text-slate-400 font-semibold mt-1 leading-relaxed">Mahasiswa atas nama <span class="text-blue-700 font-extrabold bg-blue-50 px-1.5 py-0.5 rounded border border-blue-100">Nizam Kori</span> berhasil dimasukkan ke sistem internal oleh API.</p>
                                    </div>
                                </div>
                                <div class="flex items-start gap-3.5 p-3.5 bg-gradient-to-r from-slate-50 to-white rounded-xl border border-slate-200/70 shadow-sm hover:border-purple-200 transition-colors">
                                    <div class="w-9 h-9 rounded-xl bg-purple-50 border border-purple-100 text-purple-600 flex items-center justify-center text-sm shrink-0 shadow-sm"><i class="fa-solid fa-share-nodes"></i></div>
                                    <div>
                                        <p class="text-xs font-black text-slate-800">Disposisi Berkas Pengajuan</p>
                                        <p class="text-[11px] text-slate-400 font-semibold mt-1 leading-relaxed">Aplikasi data <span class="text-purple-700 font-extrabold bg-purple-50 px-1.5 py-0.5 rounded border border-purple-100">Ahmad Firasy Rahman</span> berhasil didelegasikan ke Unit Sistem Informasi.</p>
                                    </div>
                                </div>
                            </div>
                        </div>
