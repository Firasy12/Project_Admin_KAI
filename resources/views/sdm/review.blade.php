...
<body class="flex h-screen overflow-hidden text-slate-800 bg-[#f4f6f9]">
            ...
            <ul class="space-y-2 px-3">
                ...
                <li>
                    <a href="{{ url('/sdm/review-pengajuan') }}"
                        class="flex items-center px-6 py-3 transition-all duration-200 {{ Request::is('sdm/review-pengajuan') || Request::is('sdm/review*') ? 'text-white bg-gradient-to-r from-[#f47920] to-[#e0650d] font-bold rounded-full shadow-lg shadow-orange-900/30' : 'text-slate-300 hover:text-white hover:bg-[#f47920]/15 font-medium rounded-full group' }}">
                        <i
                            class="fa-solid fa-user-shield w-5 text-center text-base mr-3 transition-colors {{ Request::is('sdm/review-pengajuan') || Request::is('sdm/review*') ? 'text-white' : 'text-slate-400 group-hover:text-[#f47920]' }}"></i>
        ...
        <header
            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xs font-semibold text-slate-400 mt-0.5">Evaluasi dan berikan keputusan validasi
                        berkas untuk calon peserta magang.</p>
                </div>
            </div>
            ...
            <div class="flex items-center space-x-4">
                <div class="flex items-center bg-slate-50 border border-slate-200 rounded-2xl p-2 pr-5 shadow-inner">
                    <div
                        class="w-9 h-9 rounded-xl bg-purple-600 text-white flex items-center justify-center font-black text-sm mr-3 shadow-md">
                        SDM
                    </div>
                    <div class="flex flex-col text-left">
                        <span class="text-xs font-extrabold text-[#0b1739] leading-tight">Admin SDM</span>
                        <span class="text-[10px] text-slate-400 font-bold leading-none mt-0.5">Human Capital
                            Center</span>
                    </div>
                </div>
            </div>
                                    ...
                                    <tr class="hover:bg-slate-50/80 transition-colors">
                                        ...
                                        <td class="px-6 py-4 text-center">
                                            @if(($item->status_raw ?? '') === 'perlu_perbaikan')
                                                <span
                                                    class="inline-flex items-center gap-1.5 text-[11px] font-extrabold px-3 py-1 bg-orange-50 text-orange-700 border border-orange-200 rounded-lg uppercase tracking-wider shadow-sm">
                                                    <i class="fa-solid fa-rotate-left text-[9px]"></i> Perlu Revisi
                                                </span>
                                            @else
                                                <span
                                                    class="inline-flex items-center gap-1.5 text-[11px] font-extrabold px-3 py-1 bg-amber-50 text-amber-700 border border-amber-200 rounded-lg uppercase tracking-wider shadow-sm">
                                        ...
                                        <td class="px-6 py-4 text-center whitespace-nowrap">
                                            <form id="reviewForm{{ $item->id }}"
                                                action="{{ url('/sdm/pengajuan/' . $item->id . '/review') }}" method="POST"
                                                class="inline-flex items-center gap-1.5">
                                                @csrf

                                                <input type="hidden" name="status" class="statusInput">
                                                <input type="hidden" name="catatan" class="catatanInput">

                                                <button type="button"
                                                    class="review-btn inline-flex items-center gap-1 px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-extrabold"
                                                    data-status="disposisi">
                                                    <i class="fa-solid fa-share-nodes"></i> Disposisi
                                                </button>

                                                <button type="button"
                                                    class="review-btn inline-flex items-center gap-1 px-3 py-1.5 bg-amber-500 hover:bg-amber-600 text-white rounded-xl text-xs font-extrabold"
                                                    data-status="perlu_perbaikan">
                                                    <i class="fa-solid fa-rotate-left"></i> Revisi
                                                </button>

                                                <button type="button"
                                                    class="review-btn inline-flex items-center gap-1 px-3 py-1.5 bg-rose-600 hover:bg-rose-700 text-white rounded-xl text-xs font-extrabold"
                                                    data-status="ditolak">
                                ...
                                @empty
                                    <tr>
                                        <td colspan="5" class="px-6 py-16 text-center text-slate-400 bg-white">
                                            ...
                                            <p class="text-xs font-semibold text-slate-400 mt-1">Belum ada pengajuan baru
                                                yang memerlukan review berkas.</p>
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
    ...
    <script>
        document.querySelectorAll('.review-btn').forEach(btn => {
            ...
            btn.addEventListener('click', async function () {
                ...
                if (status === "disposisi") {
                    textKonfirmasi = "meneruskan pengajuan ini ke unit";
                }
                ...
                if (status === "perlu_perbaikan") {
                    ...
                    const hasil = await Swal.fire({
                        ...
                        inputPlaceholder: "Contoh: Surat pengantar belum ditandatangani...",
                        inputAttributes: {
                            rows: 6
                        }, heightAuto: false,
                    ...
                    catatan = hasil.value;
                    textKonfirmasi = "meminta revisi berkas";
                }

                if (status === "ditolak") {
                    textKonfirmasi = "menolak pengajuan ini";
                }

                const konfirmasi = await Swal.fire({
                    title: "Konfirmasi",
                    heightAuto: false,
                    text: `Apakah Anda yakin ingin ${textKonfirmasi}?`,
                    icon: "question",
                    showCancelButton: true,
                ...
                form.querySelector(".statusInput").value = status;
                form.querySelector(".catatanInput").value = catatan;
    ...
    <script>
        // Auto-refresh: lihat catatan yang sama di dashboard.blade.php dkk.
        (function () {
