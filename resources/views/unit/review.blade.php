            ...
            <ul class="space-y-2 px-3">
                <li>
                    <a href="{{ url('/unit/dashboard') }}"
                        class="flex items-center px-6 py-3 transition-all duration-200 {{ Request::is('unit/dashboard') ? 'text-white bg-gradient-to-r from-[#f47920] to-[#e0650d] font-bold rounded-full shadow-lg' : 'text-slate-300 hover:text-white hover:bg-[#f47920]/15 font-medium rounded-full group' }}">
                        <i
                            class="fa-solid fa-house w-5 text-center text-base mr-3 text-slate-400 group-hover:text-[#f47920]"></i>
                        <span class="text-sm">Dashboard</span>
                    </a>
                </li>
                <li>
                    <a href="{{ url('/unit/pengajuan-masuk') }}"
                        class="flex items-center px-6 py-3 transition-all duration-200 {{ Request::is('unit/pengajuan-masuk') ? 'text-white bg-gradient-to-r from-[#f47920] to-[#e0650d] font-bold rounded-full shadow-lg' : 'text-slate-300 hover:text-white hover:bg-[#f47920]/15 font-medium rounded-full group' }}">
                        <i
                            class="fa-solid fa-file-import w-5 text-center text-base mr-3 text-slate-400 group-hover:text-[#f47920]"></i>
                        <span class="text-sm">Pengajuan Masuk</span>
                    </a>
                </li>
                <li>
                    <a href="{{ url('/unit/review-pengajuan') }}"
                        class="flex items-center px-6 py-3 transition-all duration-200 text-white bg-gradient-to-r from-[#f47920] to-[#e0650d] font-bold rounded-full shadow-lg shadow-orange-900/30">
                ...
                <li>
                    <a href="{{ url('/unit/monitoring') }}"
                        class="flex items-center px-6 py-3 transition-all duration-200 {{ Request::is('unit/monitoring') ? 'text-white bg-gradient-to-r from-[#f47920] to-[#e0650d] font-bold rounded-full shadow-lg' : 'text-slate-300 hover:text-white hover:bg-[#f47920]/15 font-medium rounded-full group' }}">
                        <i
                            class="fa-solid fa-chart-line w-5 text-center text-base mr-3 text-slate-400 group-hover:text-[#f47920]"></i>
                        <span class="text-sm">Monitoring Status</span>
                    </a>
                </li>
                <li>
                    <a href="{{ url('/unit/notifikasi') }}"
                        class="flex items-center px-6 py-3 transition-all duration-200 {{ Request::is('unit/notifikasi') ? 'text-white bg-gradient-to-r from-[#f47920] to-[#e0650d] font-bold rounded-full shadow-lg' : 'text-slate-300 hover:text-white hover:bg-[#f47920]/15 font-medium rounded-full group' }}">
                        <i
                            class="fa-solid fa-bell w-5 text-center text-base mr-3 text-slate-400 group-hover:text-[#f47920]"></i>
                        <span class="text-sm">Notifikasi</span>
                    </a>
                </li>
                <li>
                    <a href="{{ url('/unit/dokumen') }}"
                        class="flex items-center px-6 py-3 transition-all duration-200 {{ Request::is('unit/dokumen') ? 'text-white bg-gradient-to-r from-[#f47920] to-[#e0650d] font-bold rounded-full shadow-lg' : 'text-slate-300 hover:text-white hover:bg-[#f47920]/15 font-medium rounded-full group' }}">
                        <i
                            class="fa-solid fa-folder-open w-5 text-center text-base mr-3 text-slate-400 group-hover:text-[#f47920]"></i>
                        <span class="text-sm">Dokumen</span>
                    </a>
                </li>
                <li>
                    <a href="{{ url('/unit/profil') }}"
                        class="flex items-center px-6 py-3 transition-all duration-200 {{ Request::is('unit/profil') ? 'text-white bg-gradient-to-r from-[#f47920] to-[#e0650d] font-bold rounded-full shadow-lg' : 'text-slate-300 hover:text-white hover:bg-[#f47920]/15 font-medium rounded-full group' }}">
                        <i
                            class="fa-solid fa-user w-5 text-center text-base mr-3 text-slate-400 group-hover:text-[#f47920]"></i>
        ...
        <header
            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xs font-semibold text-slate-400 mt-0.5">Tentukan keputusan akhir untuk mahasiswa yang
                        masuk tahap seleksi</p>
                </div>
            </div>
                            ...
                            <tbody class="text-sm text-slate-600 divide-y divide-slate-100 bg-white">
                                @forelse($pengajuan ?? [] as $item)
                                    <tr class="hover:bg-slate-50/80 transition-colors">
                                        <td class="px-6 py-4 font-extrabold text-slate-800 border-r border-slate-100">
                                            {{ $item->nama }}</td>
                                        <td class="px-6 py-4 font-bold text-slate-500 border-r border-slate-100">
                                            {{ $item->universitas }}</td>
                                        <td class="px-6 py-4 font-bold text-slate-600 border-r border-slate-100">
                                            {{ $item->jurusan ?? 'Informatika' }}</td>
                                        <td class="px-6 py-4 text-center border-r border-slate-100">
                                            <a href="{{ url('/unit/dokumen/' . $item->id) }}"
                                                class="inline-flex items-center gap-1 text-xs font-bold px-2.5 py-1 rounded-md border transition-colors {{ ($item->is_berkas_lengkap ?? false) ? 'text-blue-500 bg-blue-50 border-blue-100 hover:bg-blue-100' : 'text-amber-600 bg-amber-50 border-amber-200 hover:bg-amber-100' }}">
                                                <i
                                                    class="fa-solid {{ ($item->is_berkas_lengkap ?? false) ? 'fa-file-pdf' : 'fa-triangle-exclamation' }}"></i>
                                                {{ ($item->is_berkas_lengkap ?? false) ? 'Lengkap' : 'Belum Lengkap' }}
                                            </a>
                                        </td>
                                        <td class="px-6 py-4 text-center border-r border-slate-100">
                                            @if(($item->status_raw ?? '') == 'diterima' || ($item->status_raw ?? '') == 'ditolak')
                                                <span
                                                    class="text-[11px] font-black text-emerald-600 bg-emerald-50 border border-emerald-200 px-2 py-0.5 rounded shadow-sm">TEREVALUASI</span>
                                            @else
                                                <span
                                                    class="text-[11px] font-black text-amber-600 bg-amber-50 border border-amber-200 px-2 py-0.5 rounded shadow-sm">PENDING</span>
                                            @endif
                                        </td>
                                        ...
                                        <td class="px-6 py-4 text-center whitespace-nowrap">
                                            @if(($item->status_raw ?? '') == 'diterima')
                                                <span
                                                    class="inline-flex items-center gap-1 text-[11px] font-extrabold px-2.5 py-1 bg-emerald-50 text-emerald-600 border border-emerald-200 rounded-lg shadow-sm uppercase tracking-wide">
                                                    <i class="fa-solid fa-circle-check text-[10px]"></i> Diterima
                                                </span>
                                            @elseif(($item->status_raw ?? '') == 'ditolak')
                                                <span
                                                    class="inline-flex items-center gap-1 text-[11px] font-extrabold px-2.5 py-1 bg-rose-50 text-rose-600 border border-rose-200 rounded-lg shadow-sm uppercase tracking-wide">
                                                    <i class="fa-solid fa-circle-xmark text-[10px]"></i> Ditolak
                                                </span>
                                            @else
                                                <span class="text-xs font-semibold text-slate-400 italic">Belum
                                                    Diputuskan</span>
                                            @endif
                                        </td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="6" class="px-6 py-16 text-center text-slate-400 bg-white">
                                            ...
                                            <p class="text-xs font-semibold text-slate-400 mt-1">Belum ada mahasiswa yang
                                                masuk tahap review.</p>
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
