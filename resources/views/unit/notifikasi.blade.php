    ...
    <main class="flex-1 flex flex-col overflow-hidden">
        ...
        <header
            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xs font-semibold text-slate-400 mt-0.5">Pembaruan sistem dan pemberitahuan terkait
                        pengajuan magang</p>
                </div>
            </div>
        ...
        <div
            ...
            <div class="max-w-4xl mx-auto relative z-10 space-y-6">
                ...
                @if(isset($jumlah_masuk) && $jumlah_masuk > 0)
                    <div
                        ...
                        <div>
                            ...
                            <p class="text-xs text-rose-600 font-semibold mt-1 leading-relaxed">Ada <b>{{ $jumlah_masuk }}
                                    pengajuan magang baru</b> dari SDM yang menunggu respon Anda. Silakan cek di menu <a
                                    href="{{ url('/unit/pengajuan-masuk') }}"
                ...
                <div
                    class="bg-white rounded-2xl shadow-xl shadow-blue-950/10 border border-blue-100/50 overflow-hidden">
                                    ...
                                    <div class="flex-1">
                                        <p class="text-xs font-semibold text-slate-700 leading-relaxed">
                                            @if($item->status_raw === 'disposisi')
                                                Pengajuan magang baru atas nama <span
                                                    class="text-blue-700 font-extrabold bg-blue-50 px-1.5 py-0.5 rounded border border-blue-100/50">{{ $item->nama }}</span>
                                                ({{ $item->universitas }}) telah diteruskan oleh SDM.
                                            @elseif($item->status_raw === 'perlu_perbaikan')
                                                Pengajuan <span
                                                    class="text-amber-700 font-extrabold bg-amber-50 px-1.5 py-0.5 rounded border border-amber-100/50">{{ $item->nama }}</span>
                                                perlu perbaikan sebelum bisa diproses lebih lanjut.
                                            @elseif($item->status_raw === 'diterima')
                                                Anda telah menyetujui pengajuan magang untuk <span
                                                    class="text-emerald-700 font-extrabold bg-emerald-50 px-1.5 py-0.5 rounded border border-emerald-100/50">{{ $item->nama }}</span>.
                                            @elseif($item->status_raw === 'ditolak')
                                                Anda telah menolak pengajuan magang dari <span
                                                    class="text-rose-700 font-extrabold bg-rose-50 px-1.5 py-0.5 rounded border border-rose-100/50">{{ $item->nama }}</span>.
                                            @else
                                                Pembaruan status untuk <span
                                                    class="font-extrabold text-slate-800">{{ $item->nama }}</span>: menjadi
                                                {{ $item->status }}.
                                            @endif
                                        </p>
                                        <span class="flex items-center gap-1 text-slate-400 text-[10px] font-bold mt-2"><i
                                                class="fa-solid fa-clock text-slate-300"></i>
                                            {{ $item->updated_at ? $item->updated_at->diffForHumans() : 'Beberapa saat yang lalu' }}</span>
                                    </div>
                                </div>
                            @endforeach
                        @else
                            <div class="flex flex-col items-center justify-center py-16 text-center text-slate-400">
                                <div
                                    ...
                                    <i class="fa-solid fa-bell-slash text-3xl text-slate-300"></i>
                                </div>
                                <p class="text-sm font-extrabold text-slate-500">Belum Ada Notifikasi</p>
                                <p class="text-xs font-semibold text-slate-400 mt-1">Notifikasi akan muncul di sini saat ada
                                    pengajuan baru.</p>
                            </div>
                        @endif
                    </div>
                ...
                @if(isset($notifikasi) && method_exists($notifikasi, 'links') && $notifikasi->hasPages())
                    <div class="mt-6 flex flex-col md:flex-row items-center justify-between gap-4">
                        ...
                        <div class="text-sm text-slate-500 font-medium">
                            Menampilkan
                            <span class="font-bold text-[#0b1739]">{{ $notifikasi->firstItem() }}</span>
                            -
                            <span class="font-bold text-[#0b1739]">{{ $notifikasi->lastItem() }}</span>
                            dari
                            <span class="font-bold text-[#0b1739]">{{ $notifikasi->total() }}</span>
                            data
                        </div>
